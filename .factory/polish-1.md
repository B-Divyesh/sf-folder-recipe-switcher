...
| F-1-3 | Added `.factory/claims.json`, 13 unique `@claim:` tests, and a runner supporting each ledger command. | Every ledger command ran separately from clean clone `71ef63e`; all 13 passed. |
...
| F-1-28 | The browser privacy statement now names storage types. | `@claim:browser-private`. |
...
| F-1-32 | Privacy copy is narrower and the complete flow is intercepted. | `@claim:browser-private`, `@claim:mit-free`; live privacy section in `evidence/live/cold-check.json`. |
